...
真机安装验证优先使用 `--build-type Debug --install`。默认 Release 构建通常会生成 `release-unsigned.apk`，如果没有配置签名，Android 会拒绝安装。
...
- 报告 APK 路径。
- 检查 adb 设备，并可选执行 `adb install -r --no-streaming`。安装前会拦截未签名 Release APK，并提示改用 Debug 或配置签名。
